...
class Dataset(Base):
    """
    간소화된 데이터셋 매핑 테이블
    - Surro API의 데이터셋 ID와 Inno 사용자 매핑만 저장
    ...
    # 핵심 매핑 정보
    surro_dataset_id = Column(Integer, nullable=False, index=True, comment="Surro API 데이터셋 ID")
    created_by = Column(String(50), nullable=False, index=True, comment="생성자 member_id")

    # 선택적 캐시 정보 (성능 최적화용)
    name = Column(String(255), nullable=True, comment="데이터셋 이름 (캐시용)")
    description = Column(Text, nullable=False, comment="데이터셋 설명 (캐시용)")
    ...
    __table_args__ = (
        ...
        # 복합 인덱스: Surro 데이터셋 ID + 사용자 ID (소유권 확인용)
        Index('idx_datasets_surro_member', 'surro_dataset_id', 'created_by'),

        # 복합 인덱스: 생성 시간 기반 조회
        Index('idx_datasets_created_member', 'created_by', 'created_at'),

        # 유니크 제약: 동일한 Surro 데이터셋을 같은 사용자가 중복 매핑하는 것 방지
        Index('idx_datasets_unique_mapping', 'surro_dataset_id', 'created_by', unique=True,
              postgresql_where=Column('deleted_at').is_(None)),

        {'comment': '사용자별 Surro 데이터넷 매핑 테이블'}
    )

    def __repr__(self):
        return f"<Dataset(id={self.id}, surro_id={self.surro_dataset_id}, member={self.created_by})>"
    ...
    @classmethod
    def create_mapping(cls, surro_dataset_id: int, member_id: str, dataset_name: str = None, description: str = None):
        """새 매핑 생성을 위한 헬퍼 메서드"""
        return cls(
