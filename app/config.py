...
class Settings:
    ...
    # KB 생성은 embedding 모델이 콜드 상태(오랜만에 호출)면 업스트림 응답까지
    # 수 분 걸릴 수 있음 (실측: 콜드 30초 초과 확인, 신고자 케이스 ~5분 /
    # 웜 상태는 5~10초로 빠름). 600s는 그 실측 최댓값(~5분)에 여유를 둔 값 —
    # 콜드스타트 상한이 정확히 얼마인지는 미확인(MLOps 확인 필요), 단발성 관측치
    # 기반이라 재조정 필요할 수 있음.
    PROXY_KB_CREATE_TIMEOUT: float = float(os.getenv("PROXY_KB_CREATE_TIMEOUT", "600.0"))
